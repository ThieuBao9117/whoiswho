...
def export_report_excel(period: str, db: Session = Depends(get_db)):
    \"\"\"Export connection details for winners of a specific month in separate sheets\"\"\"
    try:
    ...
    for winner in winners:
        ...
        sheet_title = f"{winner['emp_code']}_T{target_month}"
        ws = wb.create_sheet(title=sheet_title[:31])

        ws.append([f"CHI TIẾT KẾT NỐI - NGƯỜI THẮNG CUỘC THÁNG {target_month}/{target_year}"])
        ws.append([])
        ws.append([
